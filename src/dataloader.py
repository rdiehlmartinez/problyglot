...
def meta_collate(batch):
    ...
    Args: 
        * batch: tuple containing the following: 
            * language (str): language of the current batch of data 
    ...
    Returns: 
        * language (str): Language of batch
    ...
    """
    language, (support_samples, query_samples) = batch[0] # only 1-task per batch 

    def process_batch(batch_samples):
        """ 
        Helper function to process samples from either the support or query sets,
        and return a dictionary of input_ids, label_ids and attention_mask (i.e. the 
        expected data structure returned by meta_collate).
        """
    ...
    return (language, support_batch, query_batch)
